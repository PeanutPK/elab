...
class Task:
    """
    TASK: Help Turtle to visit his friend, Leo.
    ...
    def start(self):
        ...
        if INTERACTIVE:
            ...
            angle = randint(0,360)
            leo_x = cos(radians(angle))*distance
            leo_y = sin(radians(angle))*distance
        else:
            leo_x = int(input("Enter Leo's x: "))
            leo_y = int(input("Enter Leo's y: "))
        self.leo = Home(leo_x,leo_y,32,32,IMG_HOME,"Leo")
        stage.add_object(self.leo)
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Leo's home is at ({self.leo.x:.2f},{self.leo.y:.2f})")
