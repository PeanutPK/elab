...
IMG_LIBRARY = """

"""
...
class Task:
    """
    TASK: help guide Turtle back home while stop to buy food at the food shop
    and pick up a book at the library.  The shop will randomly appear north of
    Turtle's start location on the y-axis.  The library will randomly appear
    south of Turtle's start location on the y-axis.  And the home will
    randomly appear east (right) of Turtle's start location on the x-axis.
    """
    def start(self):
        stage.reset()
        ...
        if INTERACTIVE:
            shop_x,shop_y = 0,randint(50,300)
            lib_x,lib_y = 0,-randint(50,300)
            home_x,home_y = randint(50,300),0
        else:
            shop_x,shop_y = int(input()),int(input())
            lib_x,lib_y = int(input()),int(input())
        ...
        self.library = Home(lib_x,lib_y,32,32,IMG_LIBRARY,"library")
        stage.add_object(self.home)
        stage.add_object(self.shop)
        stage.add_object(self.library)
        stage.add_object(Road(0,0,shop_x,shop_y,5,"yellow"))
        stage.add_object(Road(0,0,lib_x,lib_y,5,"yellow"))
        stage.add_object(Road(shop_x,shop_y,lib_x,lib_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,shop_x,shop_y,5,"yellow"))
        stage.add_object(Road(home_x,home_y,lib_x,lib_y,5,"yellow"))
        if INTERACTIVE:
            print(dedent(self.__doc__).strip())
            print(f"Shop appears at ({self.shop.x:.2f},{self.shop.y:.2f})")
            print(f"Library appears at ({self.library.x:.2f},{self.library.y:.2f})")
            print(f"Home appears at ({self.home.x:.2f},{self.home.y:.2f})")
        stage.total_distance = 0
        return stage.turtle,self.home,self.shop,self.library

    def shortest_path(self):
        dist1 = ( distance(0,0,self.shop.x,self.shop.y) +
                  distance(self.shop.x,self.shop.y,self.library.x,self.library.y) +
                  distance(self.library.x,self.library.y,self.home.x,self.home.y) )
        dist2 = ( distance(0,0,self.library.x,self.library.y) +
                  distance(self.library.x,self.library.y,self.shop.x,self.shop.y) +
                  distance(self.shop.x,self.shop.y,self.home.x,self.home.y) )
    ...
    def done(self):
        ...
        if any(self.shop.contains(stop) for stop in stage.stops):
            ...
            print("FAILED")
        print("Check if Turtle stopped at the library: ", end="")
        if any(self.library.contains(stop) for stop in stage.stops):
            print("PASSED")
        else:
            print("FAILED")
