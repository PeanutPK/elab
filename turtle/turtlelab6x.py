...
if INTERACTIVE:
    try:
        ...
        if std_turtle is None or \
            ...
            print(
                f"ERROR: Invalid turtle module is found in {turtle_spec.origin}")
...
class Rectangle(namedtuple("Rectangle", "x y width height")):
    ...
    def contains(self, p: Point) -> bool:
        ...
        """
        return (((self.x - self.width / 2) < p.x < (
...
class Home(Rectangle):
    ...
    def draw(self, canvas):
        if self.image is None or ImageTk is None:
            ...
            self.photo = ImageTk.PhotoImage(image)
            item = canvas.create_image(self.x, -self.y, image=self.photo)
        item_txt = canvas.create_text(self.x, -self.y + self.height / 2 + 5,
                                      text=self.text)
...
class Task:
    ...
    and pick up a book at the library.  The shop, the library, and the home
    will randomly appear ANYWHERE on the map.
    """
    ...
    def start(self):
        ...
        if INTERACTIVE:
            ...
            shuffle(locations)
            (shop_x, shop_y), (lib_x, lib_y) = locations
        else:
        ...
        if INTERACTIVE:
            ...
            print(
                f"Library appears at ({self.library.x:.2f},{self.library.y:.2f})")
    ...
    def shortest_path(self):
        dist1 = (distance(0, 0, self.shop.x, self.shop.y) +
                 distance(self.shop.x, self.shop.y, self.library.x,
                          self.library.y) +
                 distance(self.library.x, self.library.y, self.home.x,
                          self.home.y))
        dist2 = (distance(0, 0, self.library.x, self.library.y) +
                 distance(self.library.x, self.library.y, self.shop.x,
                          self.shop.y) +
